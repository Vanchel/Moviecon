 ...
 * @author Иван Тимашов
 *
 *  Адаптер для отображения фильмов на панели трендов.
 *
 * @property itemCallback интерфейс взаимодействия с элементом списка
 */
class MoviesTrendingAdapter(private val itemCallback: ItemCallback) :
    PagingDataAdapter<Movie, MoviesTrendingAdapter.ViewHolder>(MovieDiffCallback()) {
    ...
    /**
     * Реализация [DiffUtil.ItemCallback] для [Movie], используемых в [MoviesTrendingAdapter].
     */
    ...
    /**
     * Интерфейс взаимодействия с элементом [Movie], используемый в [MoviesTrendingAdapter].
     */
